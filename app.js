...
const AppState = {
  ...
  deblocage: {},          // Programme : chapitre "vu en classe" ou non, indexé par id de CHAPITRE
  historiqueQuestions: [],
  quiz: {
    chapitreId: null,
    questions: [],
...
// ==========================================================================
// 🔓 PROGRAMME — "vu en classe" ou non, par chapitre. Déverrouillage géré
// directement par l'élève (responsabilisation), de façon transparente :
// aucun mot de passe, écran consultable par toute la famille sur l'appareil.
// Tout nouveau chapitre (ajouté plus tard dans troisieme.json) démarre
// VERROUILLÉ par défaut ; seuls les chapitres déjà présents au moment de
// l'introduction de cette fonctionnalité ont été pré-débloqués une fois,
// pour ne pas couper l'accès à la révision juste avant le Brevet.
// ==========================================================================

function estDebloque(chapitreId) {
  return AppState.deblocage[chapitreId] === true;
}

function basculerDeblocage(chapitreId) {
  AppState.deblocage[chapitreId] = !estDebloque(chapitreId);
  localStorage.setItem('dnb_deblocage_v1', JSON.stringify(AppState.deblocage));
  construireMenuMatieres();
  construireEcranProgramme();
}

function migrerDeblocageInitial() {
  // Migration unique : si la fonctionnalité n'a jamais été initialisée sur cet
  // appareil, on débloque tout ce qui existe déjà (contenu pré-Brevet), pour
  // ne rien casser. Au-delà, tout nouveau chapitre démarre verrouillé.
  const dejaInitialise = localStorage.getItem('dnb_deblocage_v1') !== null;
  if (dejaInitialise) return;

  const dataToUse = AppState.data || DATA_SECOURS;
  dataToUse.matieres.forEach(m => {
    (m.chapitres || []).forEach(c => { AppState.deblocage[c.id] = true; });
  });
  localStorage.setItem('dnb_deblocage_v1', JSON.stringify(AppState.deblocage));
}

function ouvrirMonProgramme(chapitreIdAMettreEnEvidence = null) {
  ...
  $('programme-screen').classList.remove('hidden');
  construireEcranProgramme(chapitreIdAMettreEnEvidence);
}

function construireEcranProgramme(chapitreIdAMettreEnEvidence = null) {
  const container = $('programme-liste');
  if (!container) return;
  container.innerHTML = "";
  ...
  dataToUse.matieres.forEach(m => {
    if (!m.chapitres || m.chapitres.length === 0) return;

    const titreMatiere = document.createElement('div');
    titreMatiere.className = 'category-group-title';
    titreMatiere.textContent = `${m.emoji || '📘'} ${m.label || m.id}`;
    container.appendChild(titreMatiere);

    m.chapitres.forEach(c => {
      const debloque = estDebloque(c.id);
      const row = document.createElement('div');
      row.className = 'card programme-row';
      if (c.id === chapitreIdAMettreEnEvidence) row.classList.add('programme-row-highlight');
      row.style.cssText = "display:flex; justify-content:space-between; align-items:center; padding:14px 16px; margin-bottom:10px;";
      row.innerHTML = `
        <span style="font-size:.85rem; font-weight:600; color:var(--text-primary); padding-right:12px;">${c.titre}</span>
        <label class="switch-toggle" title="${debloque ? 'Vu en classe' : 'Pas encore vu'}">
          <input type="checkbox" ${debloque ? 'checked' : ''} data-chapitre-id="${c.id}">
          <span class="switch-slider"></span>
        </label>
      `;
      row.querySelector('input').onchange = () => basculerDeblocage(c.id);
      container.appendChild(row);
    });
  });

  if (chapitreIdAMettreEnEvidence) {
    setTimeout(() => {
      const el = container.querySelector('.programme-row-highlight');
...
async function initialiserApp() {
  ...
  const savedDeblocage = localStorage.getItem('dnb_deblocage_v1');
  if (savedDeblocage) {
    try { AppState.deblocage = JSON.parse(savedDeblocage); } catch (e) { AppState.deblocage = {}; }
  }
  migrerDeblocageInitial();
  ...
  for (const [nomCategorie, listeMatieres] of Object.entries(groupes)) {
    ...
    listeMatieres.forEach(m => {
      ...
      m.chapitres.forEach(c => {
        ...
        const debloque = estDebloque(c.id);
        row.style.cssText = `padding:16px 14px; margin-top:12px; background:var(--bg-card); border-radius:12px; display:flex; flex-direction:column; gap:10px; box-shadow: var(--shadow-sm); border: 1px solid var(--border-color); ${debloque ? '' : 'opacity:0.55;'}`;

        if (debloque) {
          row.onclick = (e) => ouvrirPreQuiz(m.id, c.id, e);
        } else {
          row.onclick = (e) => { e.stopPropagation(); ouvrirMonProgramme(c.id); };
        }
        ...
        row.innerHTML = `
          <div style="display:flex; justify-content:space-between; align-items:center;">
            <span style="display:flex; align-items:center; gap:8px; font-weight:600; font-size:.88rem; padding-right:12px; text-align:left; color:var(--text-primary); line-height:1.4;">
              <span title="${debloque ? statut.label : 'Pas encore vu en classe'}" style="font-size:0.85rem; flex-shrink:0;">${debloque ? statut.icone : '🔒'}</span>
              ${c.titre}
            </span>
          ...
          <div style="padding-left:2px;">
            <span style="font-size:0.65rem; font-weight:600; color:var(--text-secondary);">Pas encore vu en classe — touche pour le débloquer dans Mon Programme</span>
          </div>
          `}
        `;
...
function goHome() {
  ...
  $('flashcards-screen').classList.add('hidden');
  if ($('oral-screen')) $('oral-screen').classList.add('hidden');
  if ($('programme-screen')) $('programme-screen').classList.add('hidden');
...
function configurerNavigation() {
  ...
  if ($('btn-mon-programme')) {
    $('btn-mon-programme').onclick = () => ouvrirMonProgramme();
  }
